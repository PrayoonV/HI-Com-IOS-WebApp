...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
   <link rel="stylesheet" href="../Lib/css/jquery-ui.css">
    ...
    <script type="text/javascript">
        ...
        function popupMFGDetail(element, productId, issueDetailId) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#modal_issue_mfg .modal-header').html('รายการ : ' + element.title);
        ...
        function editMFGDetail(mfg_no) {
            $('#hdMFGNo').val(mfg_no);

            $.ajax({
                type: "POST",
                url: "DeliveryOrder.aspx/GetMFGDetailByNo",
                data: '{mfg_no:"' + mfg_no + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
                    var data = response.d;
                    $('#txtUnitWarranty').val(data.unit_warranty);
                    $('#txtAirEndWarranty').val(data.air_end_warranry);
                    $('#txtServiceFee').val(data.service_fee);

                    $('#modal_mfg_detail').modal('show');
                }
            });
        }

        function submitMfgDetail() {
            var isValid = true;
            $("#modal_mfg_detail input[type=text]").each(function () {
                $(this).parent().parent().removeClass("has-error");
            });

            if ($('#txtUnitWarranty').val() == "") {
                isValid = false;
                $('#txtUnitWarranty').parent().parent().addClass("has-error");
            }

            if ($('#txtAirEndWarranty').val() == "") {
                isValid = false;
                $('#txtAirEndWarranty').parent().parent().addClass("has-error");
            }

            if ($('#txtServiceFee').val() == "") {
                isValid = false;
                $('#txtServiceFee').parent().parent().addClass("has-error");
            }

            if (isValid) {
                var mfg_no = $('#hdMFGNo').val();
                var unit_warranty = $('#txtUnitWarranty').val();
                var air_end_warranty = $('#txtAirEndWarranty').val();
                var service_fee = $('#txtServiceFee').val();

                $.ajax({
                    type: "POST",
                    url: "DeliveryOrder.aspx/EditMFGDetail",
                    data: "{mfg_no : '" + mfg_no + "' , unit_warranty : '" + unit_warranty + "' , air_end_warranty : '" + air_end_warranty + "' , service_fee : '" + service_fee + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        $('#modal_mfg_detail').modal('hide');
                        gridViewIssueMFG.PerformCallback();
                    }
                });
            }
    ...
    <div id="div-content">
                    ...
                    <dx:ASPxGridView ID="gridViewIssue" ClientInstanceName="gridViewIssue" runat="server" Settings-VerticalScrollBarMode="Visible"
                        ...
                        <Columns>
                            <dx:GridViewDataTextColumn Caption="MFG Detail" FieldName="id" CellStyle-HorizontalAlign="Center" Width="50px">
                                <DataItemTemplate>
                                    <a id="btnMFG" class="btn btn-mini" onclick="popupMFGDetail(this, <%# Eval("product_id")%>,<%# Eval("issue_stock_detail_id")%>)" title="<%# Eval("product_no")%>">
                                ...
                                <dx:ASPxGridView ID="gridViewIssueMFG" ClientInstanceName="gridViewIssueMFG" runat="server" Settings-VerticalScrollBarMode="Visible"
                                    ...
                                    <Columns>
                                        <dx:GridViewDataTextColumn Caption="Manage" FieldName="id" CellStyle-HorizontalAlign="Center" Width="50px">
                                            <DataItemTemplate>
                                                <a id="btnMFG" class="btn btn-mini" onclick="editMFGDetail('<%# Eval("mfg_no")%>')" title="Edit">
                                                    <i class="fa fa-pencil" aria-hidden="true"></i>
                                                </a>
                                            </DataItemTemplate>
                                        </dx:GridViewDataTextColumn>
                                        <dx:GridViewDataTextColumn FieldName="id" Visible="false" Caption="ID" VisibleIndex="3" />
                                        <dx:GridViewDataTextColumn FieldName="mfg_no" Caption="MFG No" VisibleIndex="4" Width="120px" />
                                        <dx:GridViewDataTextColumn FieldName="unit_warranty" Caption="Unit Warranty" VisibleIndex="5" Width="80px" />
                                        <dx:GridViewDataTextColumn FieldName="air_end_warranry" Caption="Air End Warranty" VisibleIndex="6" Width="80px" />
                                        <dx:GridViewDataTextColumn FieldName="service_fee" Caption="Service Fee" VisibleIndex="7" Width="80px" />
                                    </Columns>
                                </dx:ASPxGridView>
                            </div>
        ...
        <!-- modal edit mfg detail -->
        <div class="modal " id="modal_mfg_detail" role="dialog" data-backdrop="static" data-keyboard="true" tabindex="-1">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <div class="modal-title">แก้ไข MFG</div>
                    </div>
                    <div class="modal-body text-center">
                        <div class="row">
                            <div class="form-group col-xs-12 no-padding">
                                <label class="control-label col-xs-3 no-padding">Unit Warranty :</label>
                                <div class="col-xs-7">
                                    <input type="text" class="form-control isValidNumber" id="txtUnitWarranty" runat="server" />
                                </div>
                            </div>
                            <div class="form-group col-xs-12 no-padding">
                                <label class="control-label col-xs-3 no-padding">Air-End Warranty :</label>
                                <div class="col-xs-7">
                                    <input type="text" class="form-control isValidNumber" id="txtAirEndWarranty" runat="server" maxlength="10" />
                                </div>
                            </div>
                            <div class="form-group col-xs-12 no-padding">
                                <label class="control-label col-xs-3 no-padding">Service Fee :</label>
                                <div class="col-xs-7">
                                    <input type="text" class="form-control isValidNumber" id="txtServiceFee" runat="server" />
                                </div>
                            </div>
                            <div class="form-group col-xs-12 no-padding">
                        ...
                        <asp:HiddenField runat="server" ID="hdMFGNo" />
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn-addItem" onclick="submitMfgDetail()">
