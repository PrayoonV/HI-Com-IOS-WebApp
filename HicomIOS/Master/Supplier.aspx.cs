                ...
                try
                {
                    using (SqlConnection conn = new SqlConnection(SPlanetUtil.GetConnectionString()))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_supplier_add", conn))
                        {
                            ...
                            cmd.Parameters.Add("@mobile", SqlDbType.VarChar, 50).Value = Convert.ToString(row.mobile);
        ...
        public static SupplierList GetEditSupplierData(string id, int index)
        {
            ...
            try
            {
                ...
                if (dsDataSupplier.Tables.Count > 0)
                {
                    ...
                    supplierData.group_name_tha = Convert.IsDBNull(row["supplier_group_name_tha"]) ? "" : Convert.ToString(row["supplier_group_name_tha"]);
                    ...
                    supplierData.mobile = Convert.IsDBNull(row["mobile"]) ? null : Convert.ToString(row["mobile"]);
