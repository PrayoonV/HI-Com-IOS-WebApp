<%@ Page Title="" Language="C#" MasterPageFile="~/Main.master" AutoEventWireup="true" CodeBehind="AdjustProductList.aspx.cs" Inherits="HicomIOS.Master.AdjustProductList" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <dx:ASPxGridView ID="gridView" runat="server" AutoGenerateColumns="False" ClientInstanceName="gridView"
            ...
            <Columns>
                <dx:GridViewDataTextColumn Caption="Manage" FieldName="id" CellStyle-HorizontalAlign="Center" Width="40px">
                    <DataItemTemplate>
                        <a id="btnEdit" class="btn btn-mini <%# ((bool)BasePage.SESSION_PERMISSION_SCREEN.is_edit) ? "" : "" %>" onclick="editItem(<%# Eval("id")%>)" title="Edit">
                            <i class="fa fa-pencil" aria-hidden="true"></i>
                        </a>|
                        <a id="btnDelete" class="btn btn-mini <%# ((bool)BasePage.SESSION_PERMISSION_SCREEN.is_del) ? "" : "" %>" onclick="deleteItem(<%# Eval("id")%>)" title="Delete">
                            <i class="fa fa-trash-o" aria-hidden="true"></i>
                    </DataItemTemplate>
                </dx:GridViewDataTextColumn>
                <dx:GridViewDataTextColumn Caption="Clone" FieldName="id" CellStyle-HorizontalAlign="Center" Width="30px">
                    <DataItemTemplate>
                        <a id="btnClone" class="btn btn-mini <%# ((bool)BasePage.SESSION_PERMISSION_SCREEN.is_create) ? "" : "" %>"
                            onclick="clone(<%# Eval("id")%>)" title="Clone">
                ...
                <dx:GridViewDataTextColumn Caption="Print" FieldName="id" CellStyle-HorizontalAlign="Center" Width="40px">
                    <DataItemTemplate>
                        <a id="btnPrint" class="btn btn-mini" onclick="print(<%# Eval("id")%>)" title="Print">
                ...
                <dx:GridViewDataTextColumn FieldName="adjust_no" Caption="No" Width="80px" />
                <dx:GridViewDataTextColumn FieldName="adjust_status" Caption="Status" Width="80px" />
                <dx:GridViewDataTextColumn FieldName="adjust_date" Caption="Date" Width="80px" />
                <dx:GridViewDataTextColumn FieldName="quotation_no" Caption="Quotation" Width="100px" />
                <dx:GridViewDataTextColumn FieldName="customer_name" Caption="Customer" Width="150px" />
                <dx:GridViewDataTextColumn FieldName="project_name" Caption="Project" Width="100px" />
                <dx:GridViewDataTextColumn FieldName="adjust_subject" Caption="Subject" Width="200px" />
            </Columns>
    ...
    <script type="text/javascript">
        ...
        function clone(e) {
            ...
            window.location.href = "AdjustProduct.aspx?cloneId=" + key;
            $.LoadingOverlay("hide");
        }
        function print(e) {
            $.LoadingOverlay("show", {
            ...
            var id = e;
            window.open("../Report/DocumentViewer.aspx?ReportArgs=Adjust_Product|" + id, "_blank");
            $.LoadingOverlay("hide");
        }
        ...
        function addNewItem() {
            window.location.href = "AdjustProduct.aspx";
        }
        ...
        function editItem(e) {
            ...
            var key = e;
            window.location.href = "AdjustProduct.aspx?dataId=" + key;
            $.LoadingOverlay("hide");
        }
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            type: "POST",
                            url: "AdjustProductList.aspx/DeleteItem",
                            data: '{id:"' + id + '"}',
