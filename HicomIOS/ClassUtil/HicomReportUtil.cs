...
public static class HicomReportUtil
{
    ...
    public static XtraReport CreateReport(ReportType reportType, List<string> args)
    {
        ...
        switch (reportType)
        {
            ...
            case ReportType.Sale_Order_Product:
                return CreateSale_Order_ProductReport(itemID.ToString(), Convert.ToInt32(dateFrom), Convert.ToInt32(dateTo));
            ...
            case ReportType.Sale_Order_Product_Discount:
                return CreateSale_Order_Product_Discount(itemID.ToString(), Convert.ToInt32(dateFrom), Convert.ToInt32(dateTo));
    ...
    static XtraReport CreateSale_Order_ProductReport(string sales_order_no, int payment_id, int period_count)
    {
        var report = new HicomIOS.Report.Sale_Order_Product();
        report.RequestParameters = false;
        report.Parameters["parameter_sale"].Value = sales_order_no;
        report.Parameters["parameter_payment"].Value = payment_id;
        report.Parameters["parameter_period"].Value = period_count;
        return report;
    }
    ...
    static XtraReport CreateSale_Order_Product_Discount(string sales_order_no, int payment_id, int period_count)
    {
        var report = new HicomIOS.Report.Sale_Order_Product();//Sale_Order_Product_Discount();
        report.RequestParameters = false;
        report.Parameters["parameter_sale"].Value = sales_order_no;
        report.Parameters["parameter_payment"].Value = payment_id;
        report.Parameters["parameter_period"].Value = period_count;
